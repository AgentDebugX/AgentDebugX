...
--current`. For Claude Code, `SessionStart` exports the future context path,
but AgentDebugX does not create durable context or session artifacts until the
first real `UserPromptSubmit`. `--current` becomes diagnosable after that
session has produced a completed captured trace.

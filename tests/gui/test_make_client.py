"""
Tests for ``agentdebug.gui.__main__.make_client`` and ``resolve_provider``.
...
# `__main__` re-raises its own ImportError naming the extras, so tell
# importorskip to treat any ImportError as "extra missing" rather than a bug.
pytest.importorskip(
    'agentdebug.gui.__main__',
    reason='needs the gui-memory and gui-app extras',
    exc_type=ImportError,
)

from agentdebug.gui.__main__ import make_client, resolve_provider  # noqa: E402
...
class TestMakeClientOpenAICompat:
    ...
    def test_gemini(self, _clean_env) -> None:
        ...
        from agentdebug.gui.openai_adapter import OpenAICompatAdapter
    ...
    def test_sonnet(self, _clean_env) -> None:
        ...
        from agentdebug.gui.perplexity_adapter import PerplexityResponsesAdapter
...
class TestResolveAndMakeTogether:
    ...
    def test_end_to_end_routing(
        ...
        if expected_adapter_attr == "OpenAICompatAdapter":
            from agentdebug.gui.openai_adapter import OpenAICompatAdapter as Expected
        else:
            from agentdebug.gui.perplexity_adapter import PerplexityResponsesAdapter as Expected

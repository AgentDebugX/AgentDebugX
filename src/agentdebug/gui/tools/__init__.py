...
Every previously-exported symbol is preserved, so
``from agentdebug.gui.tools import TOOLS / RCA_TOOLS / DISCUSSION_TOOLS /
get_tools`` behaves exactly like the original ``debugger.tools``.
...
Three descriptors carry ``_contexts={"rca"}`` so they flow into
``RCA_WITH_LESSONS_TOOLS`` automatically; their *implementations* live in
``lesson_explorer`` next door, which needs the ``gui-memory`` extra and is
therefore resolved lazily (see ``__getattr__`` below).
"""

from ..taxonomy import TAXONOMY_CATEGORIES, ALL_SUBTYPES
...
def __getattr__(name):
    """Import ``lesson_explorer`` on first access.

    The descriptors above are plain data, but the implementations behind the
    three lesson tools pull in the lesson/episodic memory stack and its
    ``gui-memory`` dependencies. ``run_rca`` only exposes those tools when a
    ``lesson_table`` is supplied, so this never fires on the default RCA path.

    ``import_module`` rather than ``from . import lesson_explorer``: the latter
    probes the package with ``hasattr`` first, which re-enters this function.
    """
    if name == "lesson_explorer":
        from importlib import import_module

        module = import_module(f"{__name__}.lesson_explorer")
        globals()["lesson_explorer"] = module
        return module

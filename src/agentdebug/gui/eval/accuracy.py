...
def _select_human(data: dict, annotation_policy: str) -> Optional[dict]:
...
def compute_tag_metrics(pairs: list[dict]) -> dict:
    ...
    all_labels = sorted({p["human_tag"] for p in pairs} | {p["llm_tag"] for p in pairs})
    ...
    all_cats = sorted({get_category(label) for label in all_labels})
    per_category = {}
    for cat in all_cats:
...
def quick_acc(trial_dir: Union[str, Path], annotation_policy: str = "final_or_single") -> dict[str, Any]:

"""Command-line entry point for the GUI/CUA debugger pipeline.

Run as ``python -m agentdebug.gui``.

This module predates :mod:`agentdebug.gui.pipeline` and still carries its own
copy of the trial-directory, classification, result-writing and worker logic.
The two implementations have drifted (``pipeline.client_factory.make_client``
takes a ``base_url`` that :func:`make_client` here does not, and
:func:`resolve_provider` exists only here), so they are kept separate rather
than merged during the package migration.
"""
...
from .tagger import soft_tag_candidates
from .config import load_config
from .utils import ABlockLogger
...
try:
    ...
    raise ImportError(
        'The batch pipeline needs the optional GUI stack (lesson/episodic '
        'memory plus the provider SDKs). Install it with '
        '`pip install "agentdebugx[gui,gui-memory,gui-app]"`, or use the '
        'agentdebug.gui RCA API directly, which runs on a core install.'
    ) from exc
...
def _create_trial_dir(output_dir: Path, trial_name: Optional[str] = None) -> Path:
...
def make_client(provider: str, model: str):
    ...
    * ``"sonnet"``    â€” Perplexity, supports ``anthropic/claude-*`` model
    ...
    if provider in _PERPLEXITY_RESPONSES_PROVIDERS:
        from .perplexity_adapter import PerplexityResponsesAdapter
...
def print_ingestion(ir: IngestionResult, traj_dir: Path) -> None:
    ...
    if ir.status == "failure" and ir.trajectory:
        ...
        log.info("")
        log.info("Soft-tag candidates (heuristic, no LLM):")
...
def _process_single_task(
    ...
    task_index: int,
    total_tasks: int,
    lesson_mem: Optional[LessonMemory] = None,
    lesson_top_k: int = 3,
) -> Optional[dict]:
    """
    ...
    from .tagger import tag_from_rca
...
def cmd_full_pipeline(trajectory_dir: Path, provider: str, model: str,
                      output_dir: Path, trial_name: Optional[str] = None,

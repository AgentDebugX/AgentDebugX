...
All paths below are resolved against the current working directory, so run
Streamlit from the directory that holds your ``results/`` tree.
...
from agentdebug.gui.config import load_config
from agentdebug.gui.eval import compute_accuracy
from agentdebug.gui.memory.annotation_loader import load_debugger_refs
from agentdebug.gui.taxonomy import ALL_SUBTYPES, SUBTYPE_DEFINITIONS
from agentdebug.gui.trajectory import infer_instruction_from_entry

# The working directory, not the install directory: this app reads and writes a
# user's ``results/`` and ``analysis_results/`` trees, which live outside the package.
...
def _windows_long_path(path: Union[str, Path]) -> str:
...
def _remap_path(p: Optional[str]) -> Optional[str]:
...
def _find_latest_trial() -> Optional[Path]:
    """Find the most recent debugger trial directory under OUTPUT_DIR."""
    trials = _discover_debugger_trial_dirs()
    return trials[0] if trials else None


def _debugger_trial_dirs_for_agent(agent_dir: Union[str, Path]) -> list[Path]:
...
def _select_trial(label: str, trials: list[Path]) -> Optional[str]:
...
def load_rca_results(trial_dir: Optional[str] = None):
...
def load_rca_results_for_agent(agent_dir: Optional[Union[str, Path]] = None) -> list[dict]:
...
def _classification_lookup(agent_dir: Union[str, Path]) -> dict[str, dict]:
...
def _trajectory_preview(traj_path: Optional[str]) -> tuple[int, str]:
    if not traj_path:
        return 0, ""
...
def _count_trajectory_steps(traj_path: Optional[str]) -> int:
    return _trajectory_preview(traj_path)[0]


def load_annotation_results_for_agent(agent_dir: Optional[Union[str, Path]] = None) -> list[dict]:
...
def load_episodic_memory(trial_dir: Optional[str] = None):
    td = Path(trial_dir) if trial_dir else _find_latest_trial()
    if not td:
        return []
...
def _agent_dir(trial_dir: Union[str, Path]) -> Path:
...
def load_assignments(trial_dir: str) -> Optional[dict]:
...
def load_annotation(trial_dir: str, task_id: str, annotator_name: str = "") -> Optional[dict]:
...
def save_annotation(trial_dir: str, task_id: str, task: dict,
                    ...
                    chosen_debugger: Optional[str] = None) -> Path:
...
def _make_llm_client(provider: str, model: str):
    ...
    if provider == "anthropic":
        ...
        from ..together_adapter import TogetherAnthropicAdapter
...
def _load_discussion_log(trial_dir: str, task_id: str) -> Optional[dict]:
...
def _render_discussion_panel(task: dict, traj_path: str, trial_dir: str):
    ...
    from ..discuss import DiscussionSession
    ...
    if user_input:
        ...
        session: Optional[DiscussionSession] = st.session_state.get("_discuss_session")
        if session is None:
...
def main():
    ...
    all_tags = sorted({r["taxonomy_tag"] for r in results})
    sel_tags = st.sidebar.multiselect("Taxonomy Tag", all_tags, default=all_tags)

    all_apps = sorted({r.get("app_id", "unknown") for r in results})
    ...
    def _get_assigned_tasks() -> Optional[set[str]]:

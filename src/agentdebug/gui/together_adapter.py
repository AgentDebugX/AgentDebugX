...
Usage:
    from agentdebug.gui.together_adapter import TogetherAnthropicAdapter
...
class _MessagesNamespace:
    ...
    def __init__(self, together_client: "Together", default_model: str):
    ...
    def create(
        ...
        thinking: Optional[dict] = None,  # ignored — Together doesn't support extended thinking
        temperature: float = 0.1,      # determinism default for the whole pipeline
        seed: int = 42,                # determinism default for the whole pipeline
        timeout: Optional[int] = None,  # per-request timeout in seconds (None = SDK default)
        **kwargs,
    ) -> AnthropicResponse:
        """

...
Callers normally pass an explicit ``data_file`` (the pipeline points it at
``<trial_dir>/episodic.json``). The fallback store lives under the user's home
directory rather than the installed package, so it stays writable from a wheel.
...
_DATA_FILE = Path.home() / ".agentdebug" / "gui" / "episodic.json"
...
class Episode(BaseModel):
    ...
    steps_count:        int
    terminal_step:      int
    traj_path:          Union[str, os.PathLike]
    error_step:         Optional[int]
    error_context:      Optional[dict]
    agent_intention:    Optional[str]
    taxonomy_tag:       Optional[str]

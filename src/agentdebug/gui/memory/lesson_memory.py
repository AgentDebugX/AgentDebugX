...
from typing import Any, List, Callable, Tuple, TypeVar, Literal, Optional, Union
...
from ..utils import ABlockLogger, RWLock
from ..taxonomy import SUBTYPE_DEFINITIONS
...
class MapMergeRule(MergeRule):
    ...
    def __call__(self, lesson: Lesson, threshold: Optional[int] = None) -> List[uuid.UUID]:
    ...
    def merge_func(self, bucket: List[Lesson]) -> List[Lesson]:
        ...
        app = bucket[0].app_id
        confusion = sorted({c for lesson in bucket for c in lesson.confusion_set})
        refs = sorted({r for lesson in bucket for r in lesson.episodic_refs})
...
class LessonMemory:
    def __init__(
        ...
        base_url:   Optional[str] = None,
        model:      Optional[str] = None,
        db_folder:  Union[str, os.PathLike] = Path("./lesson_database"),
        log_level:  Literal["DEBUG", "INFO", "WARNING", "ERROR"] = "INFO",
        log_file:   Union[Optional[str], os.PathLike] = None
    ) -> None:
    ...
    def __iter__(self):
        ...
        for i, _doc_id in enumerate(entries["ids"]):
    ...
    def _pop(self, node_id: uuid.UUID) -> Optional[Lesson]:
    ...
    def pop(self, node_id: uuid.UUID) -> Optional[Lesson]:
    ...
    def clear(self) -> None:
        ...
        self.logger.warning(message="LessonRAG database cleared")

    def dump(self, dir: Union[str, os.PathLike], suffix: str="json") -> None:
        def _do_dump():
            for lesson in self:
        ...
        self._read(_do_dump)
        self.logger.info(message="LessonRAG database saved")

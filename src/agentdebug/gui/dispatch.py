...
def _load_lesson_explorer():
    """Import the lesson-exploration implementations lazily.

    These tools sit on the lesson/episodic memory stack, whose dependencies are
    optional. They are only reachable when ``run_rca`` is given a
    ``lesson_table``, so the import stays out of the default RCA path.
    """
    try:
        from .tools import lesson_explorer
    except ImportError as exc:
        raise ImportError(
            'The lesson-exploration tools need the lesson/episodic memory '
            'stack. Install it with `pip install "agentdebugx[gui-memory]"`, '
            'or run RCA without a lesson_table.'
        ) from exc
    return lesson_explorer
...
def dispatch_tool(
    ...
    if name == "lookup_lessons_by_taxonomy":
        lookup_lessons_by_taxonomy = _load_lesson_explorer().lookup_lessons_by_taxonomy
    ...
    if name == "search_lessons_by_app":
        search_lessons_by_app = _load_lesson_explorer().search_lessons_by_app
    ...
    if name == "follow_episodic_ref":
        follow_episodic_ref = _load_lesson_explorer().follow_episodic_ref

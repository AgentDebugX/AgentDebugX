...
from ..pipeline.client_factory import make_client

from . import TaxonomyState, AuditEntry
from .case_loader import (
    load_case_set,
    resolve_traj_dirs,
    shuffle_for_seed,
)
from .prompts import EVOLVING_RCA_SYSTEM_PROMPT
from .tools import get_evolving_tools
...
def run_full_evolving_pipeline(
    ...
    if resume and audit_trail_path.exists() and audit_trail_path.stat().st_size > 0:
        audit_entries = [
            json.loads(line)
            for line in audit_trail_path.read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
        if audit_entries:
